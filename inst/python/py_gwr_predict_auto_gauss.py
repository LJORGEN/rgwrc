...
#Bandwidth searching
opt_bw_adap = Sel_BW(coords,Y,DV,kernel='gaussian',spherical=TF_py).search(verbose=True) #great circle distances
#Run the GWR Model
gwr_adapt_python = GWR(coords,Y,DV,kernel='gaussian',opt_bw_adap,spherical=TF_py).fit() #great circle distances
#Model Summary
...
for i in range(len(pop_py_list)):
    ...
    gwr_model = GWR(coords,Y,DV,kernel='gaussian',opt_bw_adap,spherical=TF_py) #great circle distances
